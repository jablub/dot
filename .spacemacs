...
before packages are loaded."
  ...
  ;; smartparens bug https://github.com/Fuco1/smartparens/issues/1052#issuecomment-729255128
  ...
  (global-set-key (kbd "C-s-k") 'er/expand-region)
  (global-set-key (kbd "<C-s-down>") 'er/contract-region)
  (global-set-key (kbd "C-s-j") 'er/contract-region)
  ...
  ;(setq frame-title-format (list (projectile-project-name) " - %b"))
  (projectile-global-mode)




  ;; Custom clojure dev fns

  ;; Count - as a word component
  ;; (add-hook 'clojure-mode-hook #'(lambda () (modify-syntax-entry ?- "w")))

  (defun insert-ignore-next-form ()
    (interactive)
    (insert "#_"))

  (defun cider-editor-clear-repl ()
    (interactive)
  ...
  (defun cider-clear-repl-and-run-test ()
    (interactive)
    (cider-editor-clear-repl)
    (cider-test-run-test))

  (defun cider-clear-repl-and-eval-buffer ()
    (interactive)
    (save-buffer)
    (cider-editor-clear-repl)
    (cider-eval-buffer))
  ...
  (defun my-clj-hook ()
    ;(clj-refactor-mode 1)
    (local-set-key (kbd "C-s-l") 'cider-clear-repl-and-eval-buffer)
    (local-set-key (kbd "<C-s-return>") 'cider-eval-region)
    (local-set-key (kbd "C-s-h") 'cider-doc)
    (local-set-key (kbd "C-s-/") 'comment-line)
    (local-set-key (kbd "C-s-#") 'insert-ignore-next-form)
    (local-set-key (kbd "C-s-n") 'cider-repl-set-ns)
    (local-set-key (kbd "C-s-t") 'cider-clear-repl-and-run-test)
    (local-set-key (kbd "C-s-g") 'cider-find-var)
    (local-set-key (kbd "<C-s-right>") 'forward-sexp)
    (local-set-key (kbd "<C-s-left>") 'backward-sexp)
    (local-set-key (kbd "C-s-c") 'cider-editor-clear-repl)
    (local-set-key (kbd "C-s-b") 'cider-pop-back)
    (local-set-key (kbd "C-s-r") 're-frame-jump-to-reg)
    (local-set-key (kbd "C-s-o") 'helm-projectile-find-file)
    (local-set-key (kbd "C-s-f") 'projectile-grep)

    )
  

  (defun my-nrepl-hook ()
    (local-set-key (kbd "C-s-c") 'cider-repl-clear-buffer))

;  (global-company-mode)
;  (add-hook 'cider-repl-mode-hook #'company-mode)
  (add-hook 'cider-repl-mode-hook #'my-nrepl-hook)
...
;  (add-hook 'cider-mode-hook #'eldoc-mode)
  ...
  (defun tab-groups ()
    (list
     (cond
      ((string-match "\.spacemacs.*" (buffer-name)) "spacemacs")
  ...
  ;(setq tabbar-buffer-groups-function 'amits-buffer-groups)
